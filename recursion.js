  ...
  */
  ...
  function powerCalc(base, exp) {
    if(exp < 0) {
      return console.log('exponent should be greater than 0')
    }
    if(exp === 0) {
      return 1
    }
  ...
  */
  //AHHHHHHHH I'M OFF BY ONE FIX LATER
  function fibonacci(int,base) {
    if(int === 0){
      return 0
    }
    else{
      return base++ + fibonacci(int-1,base) 
    }
  }
...
*/
//e: "east", "eats", "esat", "esta", "etas", and "etsa"
//a: aest, aset, atse, aste, aets, ates
...
Write a recursive function that prints the following organization chart. Your output should be as shown below with proper indentation to show the hierarchy. You may store the org chart in an object and send that as an input to your program.
  ...
  */
  ...
  function binaryRep(int) {
    if(int === 0) {
      return ''
    }
    else {
      const binary = Math.floor(int%2)
      return binaryRep(Math.floor(int/2)) + binary
    }
  }
